...
getDatums (BlockInMode (Block (BlockHeader slotNo _ _) txs) era) =
  case era of
    ByronEraInCardanoMode   -> concatMap (go era) txs
    ShelleyEraInCardanoMode -> concatMap (go era) txs
    AllegraEraInCardanoMode -> concatMap (go era) txs
    MaryEraInCardanoMode    -> concatMap (go era) txs
    AlonzoEraInCardanoMode  -> concatMap (go era) txs
  where
    go ::
      IsCardanoEra era =>
