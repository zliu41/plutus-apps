...
module Plutus.Script.Evaluation.Dump where
...
import Plutus.Script.Evaluation.Types (Block, Checkpoint (Checkpoint),
                                       ScriptEvent (ScriptEventFailure, ScriptEventSuccess), ScriptM,
  ...
  where
    ...
    checkpoint ev ledgerState ledgerEvents = case ev of
      RollForward block _tip -> do
        streamerState <- get
        if ssCount streamerState >= blocksPerFile || length (ssEvents streamerState) >= eventsPerFile
          then do
